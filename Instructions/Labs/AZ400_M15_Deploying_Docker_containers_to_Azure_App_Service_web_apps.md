---
lab:
    title: '实验室：将 Docker 容器部署到 Azure 应用服务 Web 应用'
    module: “模块 15：使用 Docker 管理容器'
---

# 实验室：将 Docker 容器部署到 Azure 应用服务 Web 应用
# 学生实验室手册
...
1.  在容器注册表实例的 **“访问密钥”** 边栏选项卡上，标识**注册表名称**、**登录服务器**、**管理员用户** 和**密码**条目的值。
    ...
    >**备注**： 记录**注册表名称**、**登录服务器**和**密码**条目（注册表名称和管理员用户名应匹配）。你将在本实验室的后面需要它。
...
1.  实验室计算机的 Web 浏览器窗口中显示了 Azure DevOps 门户和打开的 **“将 Docker 容器部署到 Azure 应用服务 Web 应用”** 项目，在 Azure DevOps 门户最左侧的垂直菜单栏中，单击 **“管道”**。
...
1.  在右侧的 **“代理作业”** 窗格上选择 **“Web 应用部署”** 作业，在 **“代理池”** 下拉列表中选择 **“Azure Pipelines”**，然后在 **“代理规范”** 下拉列表中，选择 **“ubuntu-16.04”**。
1.  在窗格顶部，单击 **“变量”** 标头。
1.  在管道变量列表中，设置以下变量的值：
...
1.  在窗格右上角，单击 **“保存”** 按钮以保存更改，然后在再次出现提示时，单击 **“保存”**。
